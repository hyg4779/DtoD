...
export default {
  ...
  methods: {
    getTime() {
      ...
      if(years - Number(this.comment.scommentTime.slice(0,4)) > 0) {
        this.time = '오래전'
        return this.time
      }
      else {
        if(months - Number(this.comment.scommentTime.slice(5,7)) > 0) {
          this.time = '오래전'
          return this.time
        }
        else {
          if(days - Number(this.comment.scommentTime.slice(8,10)) > 0) {
            this.time = '오래전'
            return this.time
          }
          else {
